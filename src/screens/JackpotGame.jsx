...
const JackpotGames = () => {
  ...
  const marketName = location.state?.gameName || 'Jackpot Game';

  useEffect(() => {
    const fetchJackpotGameTypes = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          'https://admin.gama567.club/api/v1/jackpot-game-bid-type', 
          { mobileNo },
          {
            headers: {
              "deviceId": "qwert",
...
//const walletBalance=localStorage.getItem("walletBalance");
  const handleGameClick = (game) => {
    ...
    navigate(`/bid/jackpot/${marketId}/${game.type}`, {
