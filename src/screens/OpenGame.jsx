...
//           "https://admin.gama567.club/api/v1/game-bid-type",
//           {
//             params: {
//               mobileNo: mobileNo
//             },
  ...
  useEffect(() => {
    const fetchGameTypes = async () => {
      try {
        ...
        const res = await axios.get("https://admin.gama567.club/api/v1/game-bid-type", {
          params: { mobileNo },
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    if (openBidTime) {
      const now = new Date();
      const bidTime = new Date(now);
      const m = openBidTime.match(/(\d+):(\d+)\s*(AM|PM)/i);
      if (m) {
        ...
        if (ap === "PM" && h < 12) h += 12;
        if (ap === "AM" && h === 12) h = 0;
        bidTime.setHours(h, min, 0, 0);
        if (now > bidTime) setIsBidTimePassed(true);
      }
    }
  }, [openBidTime]);
...
const toneFor = (name = "") => {
  ...
  if (n.includes("single digit bulk") || n.includes("single digits bulk")) return "sand";     // yellow
  if (n.includes("single digit") || n.includes("single digits")) return "peach";             // peach
  if (n.includes("jodi digit bulk")) return "teal";                                          // teal
  if (n.includes("jodi digit")) return "blue";                                               // blue
  if (n.includes("single pana bulk")) return "pink";                                         // pink
  if (n.includes("single pana")) return "purple";                                            // purple
  if (n.includes("double pana bulk")) return "orange";                                       // orange
  if (n.includes("double pana")) return "green";                                             // green
  if (n.includes("sp motor")) return "aqua";                                                 // aqua
  if (n.includes("dp motor")) return "magenta";                                              // magenta
  if (n.includes("jodi")) return "magenta";
  if (n.includes("jodi bulk")) return "green";
  if (n.includes("triple pana")) return "magenta";
  if (n.includes("two digits panel")) return "magenta";

  // DEFAULT: return a random tone from the set above
  return _randFallbackTone();
};
      ...
      <div className="open-game-container">
        ...
        <div className="game-grid">
          {gameTypes.map((type) => (
            <div
              key={type.id}
              className={`game-carrd tone-${toneFor(type.name)}`}
              onClick={() => handleCardClick(type)}
              role="button"
            >
              <div className="icon-wrapper">
