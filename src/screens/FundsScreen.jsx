...
//         const response = await axios.get(`https://admin.gama567.club/api/v1/fees-settings`, {
//           data: {
...
export default function FundsScreen() {
  ...
  const handleWithdrawClick = () => {
    try {
      ...
      if (now >= openTime && now <= closeTime) {
        ...
        toast.warn(`Withdrawal is only available from ${withdrawOpenTime} to ${withdrawCloseTime}.`);
      }
    } catch (error) {
      console.error("Error during withdrawal check:", error);
      toast.error("An unexpected error occurred.");
  ...
  // Only layout changed below: 2x3 tile grid like the screenshot
  const tiles = [
    {
      icon: iconAdd,
      title: "Add Funds",
    ...
    //   tone: "mint",
    //   onClick: () => navigate("/funds/account-statement"),
    // },
    {
      icon: iconBank,
      title: "Add Bank Details",
